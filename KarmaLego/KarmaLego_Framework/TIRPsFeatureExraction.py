...
class TIRPsFeatureExtraction (object):
    ...
    def getFeaturesListByTIRP(self,tirp,entities_list,representation):
        ...
        elif representation=='ART_HS':
            feature_list=[sum(tirp._Artemis_by_entity[x]) if( x in tirp._Artemis_by_entity.keys()) else 0 for x in entities_list]
        elif representation=='ART_MD':
            for entity in entities_list:
                instances=[instances for entity_id, instances in tirp._supporting_sequences_by_entity.items() if entity_id==entity]
                Artemis = [Artemis for entity_id, Artemis in tirp._Artemis_by_entity.items() if entity_id==entity]
                if len(instances)==0:
                    feature_list.append(0)
                else:
                    ...
                    for i,instance in enumerate(instances):
                        ...
                        duration_list.append(duration*Artemis[i])
                        start_list = []
                        end_list = []
                    mean_duration=sum(duration_list)/sum(Artemis)
